...
func TestThroughput_NotNil(t *testing.T) {
	...
	require.Equal(t, 0.0,
		mustFindMetric(t, "blinkdisk_some_throughput2_bytes_total", prommodel.MetricType_COUNTER, nil).
			GetCounter().GetValue())
	require.Equal(t, 0.0,
		mustFindMetric(t, "blinkdisk_some_throughput2_duration_nanos_total", prommodel.MetricType_COUNTER, nil).
			GetCounter().GetValue())
