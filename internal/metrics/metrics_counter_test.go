...
func TestCounter_NoLabels(t *testing.T) {
	...
	require.Equal(t, 0.0,
		mustFindMetric(t, "blinkdisk_some_gauge_total", prommodel.MetricType_COUNTER, nil).
			GetCounter().GetValue())
	cnt.Add(33)
...
func TestCounter_WithLabels(t *testing.T) {
	...
	require.Equal(t, 0.0,
		mustFindMetric(t, "blinkdisk_some_gauge2_total", prommodel.MetricType_COUNTER, map[string]string{"key1": "label1"}).
			GetCounter().GetValue())
	require.Equal(t, 0.0,
		mustFindMetric(t, "blinkdisk_some_gauge2_total", prommodel.MetricType_COUNTER, map[string]string{"key1": "label2"}).
			GetCounter().GetValue())
	cnt1.Add(33)
	cnt2.Add(44)
