...
func TestAutoUpdateEnableTest(t *testing.T) {
	cases := []struct {
		...
		{desc: "DisabledByEnvar-false", extraEnv: map[string]string{"BLINKDISK_CHECK_FOR_UPDATES": "false"}, wantEnabled: false},
		{desc: "DisabledByEnvar-0", extraEnv: map[string]string{"BLINKDISK_CHECK_FOR_UPDATES": "0"}, wantEnabled: false},
		{desc: "DisabledByEnvar-f", extraEnv: map[string]string{"BLINKDISK_CHECK_FOR_UPDATES": "f"}, wantEnabled: false},
		{desc: "DisabledByEnvar-False", extraEnv: map[string]string{"BLINKDISK_CHECK_FOR_UPDATES": "False"}, wantEnabled: false},
		{desc: "DisabledByEnvar-FALSE", extraEnv: map[string]string{"BLINKDISK_CHECK_FOR_UPDATES": "FALSE"}, wantEnabled: false},
		{desc: "DisabledByEnvarOverriddenByFlag", extraEnv: map[string]string{"BLINKDISK_CHECK_FOR_UPDATES": "false"}, extraArgs: []string{"--check-for-updates"}, wantEnabled: true, wantInitialDelay: 24 * time.Hour},
		{desc: "EnabledByEnvarOverriddenByFlag", extraEnv: map[string]string{"BLINKDISK_CHECK_FOR_UPDATES": "true"}, extraArgs: []string{"--no-check-for-updates"}, wantEnabled: false, wantInitialDelay: 24 * time.Hour},

		{desc: "InitialUpdateCheckIntervalFlag", extraEnv: map[string]string{"BLINKDISK_INITIAL_UPDATE_CHECK_DELAY": "1h"}, wantEnabled: true, wantInitialDelay: 1 * time.Hour},
	...
	os.Unsetenv("BLINKDISK_CHECK_FOR_UPDATES")
	...
	for _, tc := range cases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			updateInfoFile := filepath.Join(e.ConfigDir, ".blinkdisk.config.update-info.json")
