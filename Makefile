...
TEST_FLAGS?=
BLINKDISK_INTEGRATION_EXE=$(CURDIR)/dist/testing_$(GOOS)_$(GOARCH)/blinkdisk.exe
...
BLINKDISK_BUILD_TAGS=
BLINKDISK_BUILD_FLAGS=-trimpath -ldflags "-s -w -X github.com/blinkdisk/core/repo.BuildVersion=$(BLINKDISK_VERSION_NO_PREFIX) -X github.com/blinkdisk/core/repo.BuildInfo=$(shell git rev-parse HEAD) -X github.com/blinkdisk/core/repo.BuildGitHubRepo=$(GITHUB_REPOSITORY)"

blinkdisk_ui_embedded_exe=dist/blinkdisk_$(GOOS)_$(GOARCH)/blinkdisk$(exe_suffix)

ifeq ($(GOOS),darwin)
	# on macOS, BlinkDisk uses universal binary that works for AMD64 and ARM64
	blinkdisk_ui_embedded_exe=dist/blinkdisk_darwin_universal/blinkdisk
endif

ifeq ($(GOOS),linux)

ifeq ($(GOARCH),arm)
	blinkdisk_ui_embedded_exe=dist/blinkdisk_linux_armv7l/blinkdisk
endif

ifeq ($(GOARCH),amd64)
	blinkdisk_ui_embedded_exe=dist/blinkdisk_linux_x64/blinkdisk
endif
...
install:
	go install $(BLINKDISK_BUILD_FLAGS) -tags "$(BLINKDISK_BUILD_TAGS)" github.com/blinkdisk/core

install-noui: BLINKDISK_BUILD_TAGS=nohtmlui
install-noui: install

install-race:
	go install -race $(BLINKDISK_BUILD_FLAGS) -tags "$(BLINKDISK_BUILD_TAGS)"
...
blinkdisk-ui: $(blinkdisk_ui_embedded_exe)
...
ifeq ($(GOOS)/$(GOARCH),linux/amd64)
	...
	sudo apt-get install -y ./dist/blinkdisk-ui/blinkdisk-ui*_amd64.deb
endif
	$(MAYBE_XVFB) $(MAKE) -C app e2e-test

# use this to test htmlui changes in full build of BlinkDiskUI, this is rarely needed
...
#
#   https://github.com/blinkdisk/core
#   https://github.com/blinkdisk/htmlui
#   https://github.com/blinkdisk/htmluibuild

blinkdisk-ui-with-local-htmlui-changes:
	(cd ../htmlui && npm run build && ./push_local.sh)
	rm -f $(blinkdisk_ui_embedded_exe)
	GOWORK=$(CURDIR)/tools/localhtmlui.work $(MAKE) blinkdisk-ui
...
# blinkdisk-ui build needs this particular location to embed the correct server binary.
# note we're not building or embedding HTML UI to speed up PR testing process.
build-current-os-noui:
	go build $(BLINKDISK_BUILD_FLAGS) -o $(blinkdisk_ui_embedded_exe) github.com/blinkdisk/core

# on macOS build and sign AMD64, ARM64 and Universal binary and *.tar.gz files for them
dist/blinkdisk_darwin_universal/blinkdisk dist/blinkdisk_darwin_amd64/blinkdisk dist/blinkdisk_darwin_arm6/blinkdisk: $(all_go_sources)
	GOARCH=arm64 go build $(BLINKDISK_BUILD_FLAGS) -o dist/blinkdisk_darwin_arm64/blinkdisk -tags "$(BLINKDISK_BUILD_TAGS)" github.com/blinkdisk/core
	GOARCH=amd64 go build $(BLINKDISK_BUILD_FLAGS) -o dist/blinkdisk_darwin_amd64/blinkdisk -tags "$(BLINKDISK_BUILD_TAGS)" github.com/blinkdisk/core
	mkdir -p dist/blinkdisk_darwin_universal
	lipo -create -output dist/blinkdisk_darwin_universal/blinkdisk dist/blinkdisk_darwin_arm64/blinkdisk dist/blinkdisk_darwin_amd64/blinkdisk
ifneq ($(MACOS_SIGNING_IDENTITY),)
	codesign -v --keychain $(MACOS_KEYCHAIN) -s $(MACOS_SIGNING_IDENTITY) --force dist/blinkdisk_darwin_amd64/blinkdisk
	codesign -v --keychain $(MACOS_KEYCHAIN) -s $(MACOS_SIGNING_IDENTITY) --force dist/blinkdisk_darwin_arm64/blinkdisk
	codesign -v --keychain $(MACOS_KEYCHAIN) -s $(MACOS_SIGNING_IDENTITY) --force dist/blinkdisk_darwin_universal/blinkdisk
endif
	tools/make-tgz.sh dist blinkdisk-$(BLINKDISK_VERSION_NO_PREFIX)-macOS-x64 dist/blinkdisk_darwin_amd64/blinkdisk
	tools/make-tgz.sh dist blinkdisk-$(BLINKDISK_VERSION_NO_PREFIX)-macOS-arm64 dist/blinkdisk_darwin_arm64/blinkdisk
	tools/make-tgz.sh dist blinkdisk-$(BLINKDISK_VERSION_NO_PREFIX)-macOS-universal dist/blinkdisk_darwin_universal/blinkdisk

# on Windows build and sign AMD64 and *.zip file
dist/blinkdisk_windows_amd64/blinkdisk.exe: $(all_go_sources)
	GOOS=windows GOARCH=amd64 go build $(BLINKDISK_BUILD_FLAGS) -o dist/blinkdisk_windows_amd64/blinkdisk.exe -tags "$(BLINKDISK_BUILD_TAGS)" github.com/blinkdisk/core
...
ifeq ($(GOARCH),amd64)
	$(MAKE) goreleaser
	mv dist/core_linux_arm64/core dist/core_linux_arm64/blinkdisk
	mv dist/core_linux_arm64 dist/blinkdisk_linux_arm64
	mv dist/core_linux_amd64/core dist/core_linux_amd64/blinkdisk
	mv dist/core_linux_amd64 dist/blinkdisk_linux_x64
	mv dist/core_linux_arm_6/core dist/core_linux_arm_6/blinkdisk
...
download-rclone:
	...
	go run ./tools/gettool --tool rclone:$(RCLONE_VERSION) --output-dir dist/blinkdisk_linux_arm64/ --goos=linux --goarch=arm64
	go run ./tools/gettool --tool rclone:$(RCLONE_VERSION) --output-dir dist/blinkdisk_linux_arm_6/ --goos=linux --goarch=arm
...
test-with-coverage: export BLINKDISK_COVERAGE_TEST=1
...
provider-tests: export BLINKDISK_PROVIDER_TEST=true
...
build-integration-test-binary:
	go build $(BLINKDISK_BUILD_FLAGS) $(INTEGRATION_TEST_RACE_FLAGS) -o $(BLINKDISK_INTEGRATION_EXE) -tags testing github.com/blinkdisk/core

$(TESTING_ACTION_EXE): tests/testingaction/main.go
	go build -o $(TESTING_ACTION_EXE) -tags testing github.com/blinkdisk/core/tests/testingaction

compat-tests: export BLINKDISK_CURRENT_EXE=$(CURDIR)/$(blinkdisk_ui_embedded_exe)
compat-tests: export BLINKDISK_08_EXE=$(blinkdisk08)
compat-tests: export BLINKDISK_017_EXE=$(blinkdisk017)
compat-tests: GOTESTSUM_FLAGS=--format=testname --no-summary=skipped --jsonfile=.tmp.compat-tests.json
compat-tests: $(blinkdisk_ui_embedded_exe) $(blinkdisk08) $(blinkdisk017) $(gotestsum)
	$(GO_TEST) $(TEST_FLAGS) -count=$(REPEAT_TEST) -parallel $(PARALLEL) -timeout 3600s github.com/blinkdisk/core/tests/compat_test
	#  -$(gotestsum) tool slowest --jsonfile .tmp.compat-tests.json  --threshold 1000ms

endurance-tests: export BLINKDISK_EXE ?= $(BLINKDISK_INTEGRATION_EXE)
endurance-tests: export BLINKDISK_LOGS_DIR=$(CURDIR)/.logs
endurance-tests: export BLINKDISK_TRACK_CHUNK_ALLOC=1
endurance-tests: build-integration-test-binary $(gotestsum)
	go test $(TEST_FLAGS) -count=$(REPEAT_TEST) -parallel $(PARALLEL) -timeout 3600s github.com/blinkdisk/core/tests/endurance_test

recovery-tests: export BLINKDISK_EXE ?= $(BLINKDISK_INTEGRATION_EXE)
...
recovery-tests: build-integration-test-binary $(gotestsum)
	...
	$(GO_TEST) -count=$(REPEAT_TEST) github.com/blinkdisk/core/tests/recovery/recovery_test $(TEST_FLAGS)

robustness-tests: export BLINKDISK_EXE ?= $(BLINKDISK_INTEGRATION_EXE)
robustness-tests: GOTESTSUM_FORMAT=testname
robustness-tests: build-integration-test-binary $(gotestsum)
	FIO_DOCKER_IMAGE=$(FIO_DOCKER_TAG) \
	$(GO_TEST) -count=$(REPEAT_TEST) github.com/blinkdisk/core/tests/robustness/robustness_test $(TEST_FLAGS)

robustness-server-tests: export BLINKDISK_EXE ?= $(BLINKDISK_INTEGRATION_EXE)
robustness-server-tests: GOTESTSUM_FORMAT=testname
robustness-server-tests: build-integration-test-binary $(gotestsum)
	FIO_DOCKER_IMAGE=$(FIO_DOCKER_TAG) \
	$(GO_TEST) -count=$(REPEAT_TEST) github.com/blinkdisk/core/tests/robustness/multiclient_test $(TEST_FLAGS)

robustness-tool-tests: export BLINKDISK_EXE ?= $(BLINKDISK_INTEGRATION_EXE)
...
ifeq ($(GOOS)/$(GOARCH),linux/amd64)
	$(GO_TEST) -count=$(REPEAT_TEST) github.com/blinkdisk/core/tests/tools/... github.com/blinkdisk/core/tests/robustness/engine/... $(TEST_FLAGS)
endif

socket-activation-tests: export BLINKDISK_ORIG_EXE ?= $(BLINKDISK_INTEGRATION_EXE)
socket-activation-tests: export BLINKDISK_SERVER_EXE ?= $(CURDIR)/tests/socketactivation_test/server_wrap.sh
...
ifeq ($(GOOS),linux)
	$(GO_TEST) -count=$(REPEAT_TEST) github.com/blinkdisk/core/tests/socketactivation_test $(TEST_FLAGS)
endif

stress-test: export BLINKDISK_STRESS_TEST=1
stress-test: export BLINKDISK_DEBUG_MANIFEST_MANAGER=1
stress-test: export BLINKDISK_LOGS_DIR=$(CURDIR)/.logs
stress-test: export BLINKDISK_KEEP_LOGS=1
stress-test: $(gotestsum)
	$(GO_TEST) -count=$(REPEAT_TEST) -timeout 3600s github.com/blinkdisk/core/tests/stress_test
	$(GO_TEST) -count=$(REPEAT_TEST) -timeout 3600s github.com/blinkdisk/core/tests/repository_stress_test

os-snapshot-tests: export BLINKDISK_EXE ?= $(BLINKDISK_INTEGRATION_EXE)
os-snapshot-tests: GOTESTSUM_FORMAT=testname
os-snapshot-tests: build-integration-test-binary $(gotestsum)
	$(GO_TEST) -count=$(REPEAT_TEST) github.com/blinkdisk/core/tests/os_snapshot_test $(TEST_FLAGS)

layering-test:
ifneq ($(GOOS),windows)
	# verify that code under repo/ can only import code also under repo/ + some
	# whitelisted internal packages.
	find repo/ -name '*.go' | xargs grep "^\t\"github.com/blinkdisk/core" \
	   | grep -v -e github.com/blinkdisk/core/repo \
	             -e github.com/blinkdisk/core/internal \
	             -e github.com/blinkdisk/core/issues && exit 1 || echo repo/ layering ok
endif

htmlui-e2e-test:
	HTMLUI_E2E_TEST=1 go test -timeout 600s github.com/blinkdisk/core/tests/htmlui_e2e_test -v $(TEST_FLAGS)

htmlui-e2e-test-local-htmlui-changes:
	(cd ../htmlui && npm run build)
	HTMLUI_E2E_TEST=1 HTMLUI_BUILD_DIR=$(CURDIR)/../htmlui/build go test -timeout 600s github.com/blinkdisk/core/tests/htmlui_e2e_test -v $(TEST_FLAGS)
...
goreturns:
	find . -name '*.go' | xargs goreturns -w --local github.com/blinkdisk/core
...
stage-release:
	...
	# sign RPMs
	find $(RELEASE_STAGING_DIR) -type f -name '*.rpm' -exec rpm --define "%_gpg_name BlinkDisk Builder" --addsign {} \;
...
GH_RELEASE_FLAGS=
GH_RELEASE_NAME=v$(BLINKDISK_VERSION_NO_PREFIX)
else
...
publish-homebrew:
	$(CURDIR)/tools/homebrew-publish.sh $(RELEASE_STAGING_DIR) $(BLINKDISK_VERSION_NO_PREFIX)

publish-scoop:
	$(CURDIR)/tools/scoop-publish.sh $(RELEASE_STAGING_DIR) $(BLINKDISK_VERSION_NO_PREFIX)
...
PERF_BENCHMARK_INSTANCE=blinkdisk-perf
